...
def get_sde_loss_fn(config, sde, train, reduce_mean=True, continuous=True, likelihood_weighting=True, eps=1e-5):
    ...
    def loss_fn(model, pet, mri):
        ...
        z = torch.randn_like(pet)

        mean, std = sde.marginal_prob(pet, t)
        
        perturbed_data = mean + std[:, None, None, None] * z
        ###################### TODO ######################
        if isinstance(sde, PETSDE) and config.training.joint:
            perturbed_data = torch.cat((perturbed_data, mri), 1)
        ###################### TODO ######################
        ...
        if not likelihood_weighting:
            ...
            ###################### TODO ######################
                ...
                if isinstance(sde, PETSDE) and not config.model.channel_merge:
                    z = torch.cat((z, z), 1)
                    losses = torch.square(score * std[:, None, None, None] + z)
                else:
                    losses = torch.square(score * std[:, None, None, None] + z)
