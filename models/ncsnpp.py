...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.all_resolutions = all_resolutions = [
            config.data.image_size // (2 ** i) for i in range(num_resolutions)] #(128,64)
        ...
        if progressive_input != 'none':
            input_pyramid_ch = channels # 2

        modules.append(conv3x3(channels, nf)) #(2,128)
        hs_c = [nf]
        ...
        if self.config.model.channel_merge:
            modules.append(conv3x3(channels, 1))
        #########################################
    ...
    def forward(self, x, time_cond):
        ...
        m_idx = 0
        if self.embedding_type == 'fourier': # DO
            # Gaussian Fourier features embeddings.
        ...
        hs = [modules[m_idx](x)]  # weight: [1，128，128，128]
        m_idx += 1
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                m_idx += 1
                if h.shape[-1] in self.attn_resolutions: # not do
                    h = modules[m_idx](h)
                    m_idx += 1
