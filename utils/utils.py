...
def save_mat_label(save_dict, variable, file_name, file_name_str, index=0, normalize=True):
    # variable = variable.cpu().detach().numpy()
    if normalize:
        # variable_abs = np.absolute(variable)
        # max = np.max(variable_abs)
    ...
    file = os.path.join(save_dict, str(file_name_str)[2:15] + '_' + str(file_name) + '.mat')
    datadict = {str(file_name): np.squeeze(variable)}
    scio.savemat(file, datadict)
